...
class MainActivity : AppCompatActivity() {
    ...
    fun convertButton(View: View) {
        ...
        if (doll == null && rubl == null) {
            val toast =
                Toast.makeText(applicationContext, "оба из полей пустые", Toast.LENGTH_LONG)
            toast.show()
        } else if (rubl == null) {
            ...
            if (doll == null ) {
                val toast =
                    Toast.makeText(applicationContext, "курс обмена не указан", Toast.LENGTH_LONG)
                toast.show()
            } else {
                val formula: Int? = rubl?.div(doll)
